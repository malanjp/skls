...
- **Paging and sort direction** — `Ctrl+F` / `Ctrl+B` (and PageDown / PageUp) move by a page; `Ctrl+H` / `Ctrl+L` (and Home / End) jump to first / last; `S` toggles asc/desc
...
- List paging: `Ctrl+F` / `PageDown` forward, `Ctrl+B` / `PageUp` back (clamped; `j`/`k` still wrap). Step is the visible list height minus one row. `Ctrl+H` / `Home` jumps to the first row, `Ctrl+L` / `End` to the last
