...
- 異なるプロジェクトの同名スキルは別行。一覧に `PROJECT` 列（ディレクトリ名。ユーザースコープは `-`）
- 左サイドバーにスキャン対象プロジェクトを列挙。選ぶとそのルート配下のスキルだけを表示する。出所ナビとプロジェクトの間に選択できない仕切りを置く。プロジェクトスコープの追加・更新は cwd / `--project-root` を使う

### 変更

- スキル / プラグイン / MCP 一覧の列幅を表示内容に合わせて伸ばす（`NAME` / `PROJECT` / `SRC` / `AUTHOR` とプラグイン・MCP の相当列）。余った幅は名前列に足す

### 修正

- サイドバーのプロジェクト件数を毎描画で再計算しない。初回のプロジェクト発見はホーム全体ではなく上記のシードディレクトリだけを見る
