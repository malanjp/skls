...
- **左サイドバー** — agent / gh / npx / plugins / mcp で一覧を分割。`h`/`l`（Tab）でフォーカス、サイドバー上の `j`/`k` で切替。`t` でも循環。スキルは出所で排他分割（agent = manual + プラグイン同梱、gh / npx はそれぞれのインストーラ）
...
- 左サイドバー（`nav`）: `agent` / `gh` / `npx` / `plugins` / `mcp`。件数付き。`h`/`l` または Tab でフォーカス。gh / npx ナビからの追加は backend 選択を飛ばす
